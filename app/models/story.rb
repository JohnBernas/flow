class Story < ActiveRecord::Base
  ...
  attr_accessor :tracker_labels, :tracker_state
  store_accessor :data, :started_at
  store_accessor :tracker, :current_state, :name, :url, :owned_by, :estimate

  scope :ordered, ->{ order(:column_id, :priority) }
  scope :same_swimlane_as_story, ->(story) do
    # RankedModel gem also scopes before the record is saved. In this case, the
    # scope should be skipped, because `Label.new(story)` will return nil
    # (amongst other errors that could pop up). See http://git.io/uWxSuQ
    story.id ? scope_stories_based_on_swimlane_of_story(story) : self
  end

  def as_json(options)
    super(only: [:id, :column_id, :tracker], methods: %w[pid sid labels])
  end
  ...
  def labels
    tracker['labels'] ? tracker['labels'].split(',') : []
  end
